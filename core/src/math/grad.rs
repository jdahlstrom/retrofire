...
impl<T: Lerp> Gradient2<T> {
    ...
    pub fn new(
        ...
        Self { kind, stops: Stops::new(stops) }
    }
...
mod tests {
    ...
    fn conical_gradient() {
        ...
        // left
        assert_eq!(g.eval(pt2(-1.0, -1.0)), 0.32857156);
        assert_eq!(g.eval(pt2(-100.0, -1.0)), 0.32857156);
    ...
    fn t_less_than_or_eq_to_min_gives_min() {
        let g = Stops(vec![(0.2, -1.23f32), (0.8, 1.23f32)]);
    ...
    fn nonincreasing_t_gives_sharp_change() {
        let g = Stops(vec![(0.4, -1.23f32), (0.4, 1.23f32)]);
