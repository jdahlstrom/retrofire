...
/// # Examples
/// ```
/// # use retrofire_core::util::{buf::*, dims::Dims};
...
impl<T> Buf2<T> {
    ...
    pub fn new_from<I>(dims @ Dims(w, h): Dims, init: I) -> Self
    where
    ...
    pub fn new(dims @ Dims(w, h): Dims) -> Self
    where
        T: Clone + Default,
    {
    ...
    pub fn new_with<F>(dims @ Dims(w, h): Dims, init_fn: F) -> Self
    where
...
impl<'a, T> Slice2<'a, T> {
    ...
    /// # Examples
    /// ```
    /// # use retrofire_core::util::{buf::Slice2, dims::Dims};
    ...
    ///
    pub fn new(dims: Dims<u32>, stride: u32, data: &'a [T]) -> Self {
...
mod inner {
    ...
    impl<T, D: Deref<Target = [T]>> Inner<T, D> {
        ...
        #[rustfmt::skip]
        pub(super) fn new(dims @ Dims(w,h): Dims, stride: u32, data: D) -> Self {
    ...
    impl<T, D: Deref<Target = [T]>> Index<usize> for Inner<T, D> {
        ...
        fn index(&self, i: usize) -> &[T] {
            ...
            &self.data[idx..][..self.width() as usize]
        }
    }
