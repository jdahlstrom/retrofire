...
fn triangle(b: Bencher, n: u32) {
    let verts = [
        ...
        vertex(pt3(0.0, 1.0, 0.0), rgb(0.0, 0.8, 0.0)),
        vertex(pt3(1.0, -1.0, 0.0), rgb(0.4, 0.4, 1.0)),
    ];
    ...
    let viewport = viewport(pt2(0, dims.1)..pt2(dims.0, 0));
    ...
    assert_eq!(center_pixel, rgba(89, 127, 64, 255));

    pnm::save_ppm("benches/out/e2e_triangle.ppm", framebuf).unwrap();
}
...
fn sphere(b: Bencher, res: u32) {
    ...
    pnm::save_ppm(format!("benches/out/e2e_sphere_{res}.ppm"), framebuf)
        .unwrap();
}
