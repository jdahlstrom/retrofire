...
fn random_mat2(rng: &mut DefaultRng) -> Mat2 {
    let x = VEC2S.sample(rng);
    let y = x.perp();
    Mat2::new([x.0, y.0])
}
fn random_mat3_lin(rng: &mut DefaultRng) -> Mat3<(), (), 3> {
    let x = VEC3S.sample(rng);
    let y = Vec3::Z.cross(&x);
    let z = x.cross(&y);
    Mat3::new([x.0, y.0, z.0])
}
fn random_mat3_aff(rng: &mut DefaultRng) -> Mat3 {
    let x = VEC2S.sample(rng);
    let y = x.perp();
    let o = VEC2S.sample(rng).to_pt();
    Mat3::from_affine(x, y, o)
}
fn random_mat4(rng: &mut DefaultRng) -> Mat4 {
    let x = VEC3S.sample(rng);
    let y = Vec3::Z.cross(&x);
    let z = x.cross(&y);
