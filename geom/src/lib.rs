...
use retrofire_core::{
    geom::{Mesh, Polygon, Tri, Vertex2, Winding, tri, vertex},
    math::{Point2, pt3},
};
...
pub fn triangulate(poly: &Polygon<Point2>) -> Mesh<()> {
    let pts = &poly.0;
    let len = pts.len();
    if len < 3 {
        cold_path();
    ...
    let mut res = Mesh::new(
        ...
        pts.iter()
            // TODO Should be able to return a "Mesh2" with points intact
            .map(|pt| vertex(pt3(pt.x(), 0.0, pt.y()), ())),
    );

    // A very fast path for convex polygons (O(n) vs O(n²))
    if poly.is_convex() {
        res.faces
            .extend((2..len).map(|i| tri(0, i - 1, i)));
        return res;
    }
    ...
    for n in 0..len * len {
        if let Some(tri) = clipper.clip_if_ear(n % clipper.ixs.len()) {
            res.faces.push(tri);
            if clipper.ixs.is_empty() {
                return res;
            }
        }
    }
...
impl<B> Clipper<'_, B> {
    fn clip_if_ear(&mut self, i: usize) -> Option<Tri<usize>> {
        ...
        if self.is_convex(j) && !self.contains_point(j) {
            let k = self.next(j);
            // Valid ear -> clip it
            let res = tri(self.ixs[i], self.ixs[j], self.ixs[k]);
            self.ixs.remove(j);
            Some(res)
        } else {
            None
    ...
    fn contains_point(&self, j: usize) -> bool {
        ...
        // TODO Tri<Point2> should have contains()
        let tri: Tri<Vertex2<_, B>> =
            Tri([i, j, k].map(|h| vertex(pts[ixs[h]], ())));
    ...
    fn next(&self, i: usize) -> usize {
        if i + 1 == self.ixs.len() { 0 } else { i + 1 }
    }

    #[inline]
    fn prev(&self, i: usize) -> usize {
        (if i == 0 { self.ixs.len() } else { i }) - 1
    }
}
