...
use re::render::{
    ...
    light::{Kind, Light},
    Model, ModelToView, View, ViewToProj,
};
...
fn main() {
    ...
    let crate_shader = Shader::new(
        |v: Vertex3<Normal3>, u: &Uniform| {
            ...
            let pos = u.mv.apply_pt(&v.pos);
            let light_col = u.light.eval(pos);
            let light_dir = (u.light.pos - pos).normalize();

            let amb = gray(0.1);
            let diff = 0.9;
            let lam = diff * n.dot(&light_dir).max(0.0);

            let color = light_col.mul(lam).add(&amb);
    ...
    win.run(|frame| {
        ...
        let light_tf = translate(vec3(0.0, 0.0, 0.0))
            .then(&rotate_y(rads(frame.t.as_secs_f32())))
            .to();

        let light = light.transform(&light_tf.then(&cam.world_to_view()));
